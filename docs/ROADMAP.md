...
| 5 — Live-feed poller | ✅ **done & live** | `backend/data/fetchers/live.py` polls api-football `/fixtures?live=all` every 30s, ingests score/elapsed/red cards/possession/shots/xG, resolves to our `match_id` via existing `TEAM_IDS` map, recomputes WP via the restart simulator, appends LiveWpHistory tick if state changed, auto-marks match `complete` on FT. Pro tier verified live: Plan: Pro, 196/7500 used. |
| 6 — SSE stream endpoint | ✅ **done & live** | `GET /live/match/<id>/stream` emits `tick`, `state`, `ended` events through sse-starlette (ping=15s). Cloudflare + nginx-proxy-manager pipeline verified earlier (commit 853bd53). |
| 7 — Swing chart frontend | ✅ **done & live** | Hand-rolled SVG, three smoothed lines (home/draw/away), event markers, "now" indicator. Live stats strip (possession/shots/xG). Auto-reconnects, handles pre-match/live/complete states internally. Wired into match/[id]/page.tsx above the verdict. |
| 8 — Match page integration | ✅ done | Component renders unconditionally — handles empty/pre-match copy internally. |
| 9 — "Save this moment" share | ☐ next session | Lower priority — chart works without it |
| 14 — Big-swing push trigger | ✅ **done & live** | When live WP swings >=15pts between ticks, a push fires with title="Home X-Y Away", body="<mover> up to N% live — t'". Dedup by `(match, minute, score)` so one goal never doubles. Routes to /match/<id>. |
