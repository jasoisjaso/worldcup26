...
export function SearchBar() {
  ...
  // Flat list for keyboard navigation. Teams first then players, matching
  // visual order in the panel so up/down feels natural.
  ...
  // Open on Cmd/Ctrl+K. Standard cross-app search shortcut, costs nothing.
  useEffect(() => {
  ...
  // Focus input + lock body scroll when panel opens.
  useEffect(() => {
    if (!open) return
  ...
  return (
    <>
      <button
        ...
        className="p-1.5 rounded-lg text-slate-500 hover:text-emerald-300 hover:bg-surface-2 transition-colors"
      ...
      {open && (
        ...
        >
          <div
            className="absolute left-1/2 top-[12vh] -translate-x-1/2 w-[92%] max-w-xl rounded-2xl border border-edge bg-surface-1 shadow-2xl overflow-hidden"
